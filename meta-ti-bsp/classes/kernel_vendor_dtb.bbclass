...
python __anonymous () {
    ...
    # vmlinux.gz in kernel_vendor_dtb_do_transform_kernel()).
...
kernel_vendor_dtb_do_transform_bundled_initramfs() {
...
kernel_vendor_dtb_do_transform_kernel() {
	# vmlinux.gz is not built by kernel
	if (echo "${KERNEL_IMAGETYPES}" | grep -wq "vmlinux\.gz"); then
...
kernel_vendor_dtb_do_configure() {
...
inherit kernel_devicetree_vendor_dtb
